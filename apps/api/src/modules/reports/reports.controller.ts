...
function value(key:string, raw:unknown, isPdf = false) { if (raw === null || raw === undefined) return "—"; if (typeof raw === "boolean") return raw ? "Active" : "Inactive"; if (/(?:minutes|overtime)/iu.test(key) && /^-?\d+(\.\d+)?$/u.test(String(raw))) return minutes(raw); if (financial.test(key) && /^-?\d+(\.\d+)?$/u.test(String(raw))) return `${isPdf ? "Rs. " : "₹"}${new Intl.NumberFormat("en-IN", { minimumFractionDigits: 2, maximumFractionDigits: 2 }).format(Number(raw))}`; return String(raw).replaceAll("_", " "); }
...
function employeePdf(data: any, query: Request["query"]) {
  ...
  const fmt = (v: any) => `Rs. ${new Intl.NumberFormat("en-IN", { minimumFractionDigits: 2, maximumFractionDigits: 2 }).format(Number(v ?? 0))}`;
