...
export async function updatePunchIgnore(ids:number[],ignored:boolean){if(!ids.length)throw new Error("Select at least one raw punch");const dates=await pool.query<{date:string}>("SELECT DISTINCT (punch_time AT TIME ZONE 'Asia/Kolkata')::date::text date FROM raw_attendance_punches WHERE id=ANY($1::bigint[])",[ids]);const r=await pool.query("UPDATE raw_attendance_punches SET ignored=$2,ignored_at=CASE WHEN $2 THEN now() ELSE NULL END WHERE id=ANY($1::bigint[])",[ids,ignored]);for(const row of dates.rows)await rebuildAttendanceForAllActiveEmployees(row.date);return{updated:r.rowCount??0};}
...
export async function reprocessPunches(ids:number[]){if(!ids.length)throw new Error("Select at least one raw punch");const rows=await pool.query("SELECT DISTINCT (punch_time AT TIME ZONE 'Asia/Kolkata')::date::text date FROM raw_attendance_punches WHERE id=ANY($1::bigint[]) AND ignored=false",[ids]);for(const row of rows.rows)await rebuildAttendanceForAllActiveEmployees(row.date);await pool.query("UPDATE raw_attendance_punches SET processed=true,processed_at=now(),failure_reason=NULL WHERE id=ANY($1::bigint[]) AND ignored=false",[ids]);return{processed:rows.rowCount??0,skipped:ids.length-(rows.rowCount??0),failed:0};}
