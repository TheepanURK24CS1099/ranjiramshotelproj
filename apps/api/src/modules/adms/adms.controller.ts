...
export async function devicecmd(req:Request,res:Response,next:NextFunction){try{await receive(identity(req),"",req.ip??null);res.type("text/plain").send("OK")}catch(e){if(e instanceof AdmsDeviceError){res.status(e.statusCode).type("text/plain").send("ERROR");return}next(e)}}
