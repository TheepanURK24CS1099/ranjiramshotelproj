...
export async function bulkStatus(req:Request,res:Response,next:NextFunction){try{if(!Array.isArray(req.body?.ids)||!req.body.ids.length||typeof req.body.active!=="boolean")throw new Error("Select shifts and a status");res.json(await shiftsService.bulkShiftStatus(req.body.ids,req.body.active));}catch(e){next(e);}}
export async function bulkDeleteUnused(req:Request,res:Response,next:NextFunction){try{if(!Array.isArray(req.body?.ids)||!req.body.ids.length)throw new Error("Select at least one shift");res.json(await shiftsService.deleteUnusedShifts(req.body.ids));}catch(e){if(e instanceof Error&&e.message.startsWith("Cannot delete")){res.status(409).json({message:e.message});return;}next(e);}}
