...
it("one punch after buffer is missing",async()=>{await reset();now("12:46:00Z");await punch("03:35:00Z","late");expect(await rebuild()).toMatchObject({status:"PRESENT",note:"Missing punch out"});});
...
it("historical one punch is missing",async()=>{await reset();process.env.ATTENDANCE_TEST_NOW="2994-06-11T13:00:00Z";await punch("03:35:00Z","history-one");expect((await rebuild()).status).toBe("PRESENT");});
